...
def main() -> int:
    ...
    require("type: boolean" in workflow, "acknowledge input must be boolean")
    require("default: false" in workflow, "acknowledge default must stay false")
    require("inputs.acknowledge_single_run == true" in workflow, "acknowledgement must use inputs boolean context")
    require("github.event.inputs.acknowledge_single_run" not in workflow, "github.event.inputs must not drive acknowledgement")
    require("acknowledge_single_run" in workflow and "must be true" in workflow, "acknowledgement guard missing")
    require('re.fullmatch(r"[0-9]{14}", timestamp)' in workflow, "timestamp guard missing")
    require("max_candidates must be between 1 and 100" in workflow, "max_candidates guard missing")
    require("run_control.json" in workflow, "run control artifact missing")
    require("failure_report.json" in workflow and "failure_report.md" in workflow, "failure report artifacts missing")
    require("not_evaluated" in workflow, "summary must use not_evaluated state")
    require("scripts/review_gdelt_webngrams_candidates.py" in workflow, "live review step missing")
    require("hashFiles('artifacts/global_news_webngrams_probe/report.json') != ''" in workflow, "review must require probe report")
    ...
    require("if-no-files-found: error" in workflow, "artifact upload must error if run_control is absent")
    ...
    require("--retry" not in lowered and "max-attempts" not in lowered, "workflow must not contain request retry")
    require("fallback timestamp" not in lowered and "fallback_timestamp" not in lowered, "workflow must not contain timestamp fallback")
