...
공개 앱의 홈 화면은 매일 확인할 사업과 시장 뉴스를 빠르게 고를 수 있도록 영업 브리핑으로 구성합니다.

- **오늘의 영업 브리핑:** 진행 중 사업, 마감 7일 이내 사업, 최근 7일 신규 사업, 우선 검토 사업, 최근 7일 뉴스를 표시합니다.
- **사업 우선순위:** 우선 검토 여부와 실제 검토 시점을 분리합니다. 검토 시점은 마감일까지 남은 기간에 따라 `즉시 검토`, `이번 주 검토`, `검토 예정`, `중장기 검토`, `마감`으로 표시하고, 화면에는 `마감 D-3`, `기존 중요 사업`, `최근 등록`, `직접 관련`, `고액 사업`처럼 선정 이유를 함께 보여줍니다.
- **관심목록:** 관심 사업과 관심 뉴스는 현재 브라우저의 `localStorage`에 ID만 저장합니다. 로그인, 서버 저장, 기기 간 동기화는 지원하지 않습니다.
- **최근 본 항목:** 상세 화면에 들어간 사업과 뉴스의 ID를 최근 본 항목으로 저장합니다. 데이터 전체를 브라우저 저장소에 복사하지 않습니다.
...
뉴스의 `publisher_country_code`, `publisher_country_name`, `publisher_country_confidence`, `publisher_country_reason`은 발행 언론사 국가를 나타내는 파생 메타데이터입니다. 국가 코드는 ISO 3166-1 alpha-2를 사용하고, 국가명은 화면 표시용 한국어명을 저장합니다. `news.google.com` 같은 중계 도메인은 발행 언론사 도메인으로 쓰지 않으며, 국가를 안전하게 확정할 수 없으면 `국가 미확인`으로 둡니다. 현재 화면의 기본 필터는 계속 `전체 | 국내 | 해외`이며, 국가별 필터 UI는 후속 단계에서 추가합니다. `scripts/audit_news_publisher_country.py`는 국가 코드·국가명 정합성, 국내/해외 충돌, 미확인 국가 표본을 감사하고 workflow artifact로 남깁니다.
### 기존 로컬 대시보드
...
- known important bid 중 현재 검토 가능한 항목은 `우선 검토` 배지로 구분됩니다.
...
- 기존 중요공고는 항상 유지합니다.
- 기존 사업정보는 마감일만으로 자동 삭제하지 않고 opportunity_status로 active/closed/unknown 상태를 계산해 누적 보존합니다.
...
Candidate review is a read-only quality layer for Web NGrams probe artifacts. It does not call GDELT, does not download Web NGrams or GAL files, and does not update public JSON or the database.

Fixture review:

```bat
.\.venv\Scripts\python.exe scripts\review_gdelt_webngrams_candidates.py --fixture
```
...
## Overseas modular news collectors

`OverseasRssNewsCollector` is the default operating collector for overseas modular building news. It reads public RSS or Atom metadata, filters titles and summaries for high-precision modular construction relevance, and writes matching items through the existing collector runner, SQLite upsert, and public JSON export path. It does not fetch article pages, resolve Google News redirects, require login, or use an API key.

`GdeltDocNewsCollector` remains in the codebase as an optional DOC 2.0 Article List collector, but it is disabled by default after repeated HTTP 429 responses. It should not be repeatedly dry-run while rate-limited.

Environment variables:

- `OVERSEAS_RSS_NEWS_ENABLED`: default `true`.
- `OVERSEAS_RSS_NEWS_TIMEOUT_SECONDS`: default `20`.
- `OVERSEAS_RSS_NEWS_LOOKBACK_DAYS`: default `14`.
- `OVERSEAS_RSS_NEWS_MAX_ITEMS_PER_FEED`: default `100`.
- `OVERSEAS_RSS_NEWS_MIN_RELEVANCE_SCORE`: default `70`.
- `OVERSEAS_RSS_NEWS_FEEDS`: optional semicolon-separated `name|url` feed override.
- `GDELT_DOC_NEWS_ENABLED`: default `false`.
...
```bat
python scripts\collect_overseas_rss_news.py --dry-run --show-limit 20
...
```bat
python scripts\collect_overseas_rss_news.py --apply
python scripts\collect_gdelt_doc_news.py --apply --timespan 7d --max-records 250
```

The scheduled `Update public data` workflow runs daily at 22:00 UTC, which is 07:00 KST the next day. It enables the RSS collector and disables the GDELT DOC collector without adding a new secret. If an RSS feed fails, the collector records the feed error and continues with the next feed; only an all-feed failure fails the collector. Existing DB rows and public JSON are not treated as deleted. The Web NGrams live verification workflow remains an optional diagnostic and shadow-quality path, not the operating news collector.

Overseas RSS publication removes both canonical URL duplicates and content duplicates that share the same normalized title and publication day. This content deduplication is limited to `source == "해외 모듈러 RSS"`; domestic news keeps the normal URL-based merge policy. The publication audit still runs after deduplication as the final gate. If the audit fails, public JSON is not committed, and the audit JSON/Markdown can be inspected from the `overseas-rss-audit-*` workflow artifact.
