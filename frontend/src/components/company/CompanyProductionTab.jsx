...
export default function CompanyProductionTab({ company }) {
              ...
              {production.map((facility) => {
                ...
                return (
                  <tr key={facility.facility_id || facility.facility_name}>
                    <th>
                      <details className="company-row-detail">
                        ...
                        <dl>
                          ...
                          <div><dt>생산 대상</dt><dd>{(facility.production_scope || []).map((item) => labelValue(item, item)).join(", ") || "확인되지 않음"}</dd></div>
                          <div><dt>주요 공정</dt><dd>{(facility.production_processes || []).map((item) => labelValue(item, item)).join(", ") || "확인되지 않음"}</dd></div>
                    ...
                    <td>{formatArea(facility.site_area ?? facility.site_area_m2, facility.site_area_unit || "m2")}</td>
                    <td>{isTargetCapacity ? <><span className="mini-status-badge">목표</span> {capacity}</> : capacity}</td>
                    <td>{labelValue(facility.verification_status || facility.own_facility_status)}</td>
                  </tr>
                );
              })}
            </tbody>
