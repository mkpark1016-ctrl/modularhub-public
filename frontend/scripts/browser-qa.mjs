...
try {
  ...
  check(businessData.items.length === 105, `사업정보 기대 건수 불일치: ${businessData.items.length}`);
  check(newsData.items.length === 396, `뉴스 기대 건수 불일치: ${newsData.items.length}`);
  ...
  check(homeText.includes("105건"), "홈 사업정보 건수 누락");
  check(homeText.includes("396건"), "홈 뉴스 건수 누락");
  ...
  for (const label of ["기관", "게시일", "마감일", "금액", "공고/판단번호", "공식 확인 사이트", "검색 조합"]) {
  ...
  await page.locator("article.result-card").first().waitFor();
  check(await countCards(page) === 396, "뉴스 카드 수 불일치");
